...
<div x-data="{ addModalOpen: false, editModalOpen: false, editBan: {} }" class="space-y-6">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="p-4">
                            <div class="w-14 h-10 rounded-lg overflow-hidden border border-slate-200 bg-slate-100 shrink-0">
                                <img src="{{ Str::startsWith($b->image, 'http') ? $b->image : asset($b->image) }}" alt="Banner" class="w-full h-full object-cover">
                            </div>
                        </td>
    ...
    <div x-show="addModalOpen" x-cloak class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
        <div @click.away="addModalOpen = false" class="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-5">
            ...
            <form action="{{ route('admin.banners.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">Nama File / URL Gambar Banner</label>
                    <input type="text" name="image" required placeholder="hero-doctor.png / banner-igd.png" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-[#0e7c47] outline-none">
                </div>
    ...
    <div x-show="editModalOpen" x-cloak class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
        <div @click.away="editModalOpen = false" class="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-5">
            ...
            <form :action="'{{ url('admin/banners') }}/' + editBan.id" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label class="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1">Nama File / URL Gambar Banner</label>
                    <input type="text" name="image" x-model="editBan.image" required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-2 focus:ring-[#0e7c47] outline-none">
                </div>
